...
Each has a unique primary designation, an optional unique name,
an optional diameter, and a flag for whether the object
...
and a relative approach velocity.

A `NearEarthObject` maintains a collection of its close approaches, and a
`CloseApproach` maintains a reference to its NEO.

The functions that construct these objects use information extracted from
the data files from NASA, so these objects should be able to handle all of
the quirks of the data set, such as missing names and unknown diameters.
"""
import math
...
class NearEarthObject:
    ...
    def __init__(self, designation, name=None, diameter=None, hazardous=False):
        ...
        :param hazardous: (bool) Is Neo marked as potentially hazardous.
        """
        self.designation = designation
        assert self.designation is not None and self.designation != '', \
            f"Attribute designation is required. " \
            f"Value: '{designation}' is not allowed"
        self.name = None if name is None else name
        if math.isnan(float(diameter)):
    ...
    def _str_hazardous(self):
        """Return a human readable representation of 'hazardous'."""
        if self.hazardous:
            return "is"
        return "is not"

    def str_diameter(self):
        """Return a human readable representation of 'diameter' attribute."""
        if math.isnan(float(self.diameter)):
            return "an uknown diameter"
        return f"a diameter of {self.diameter: .3f} km"

    def __str__(self):
        """Return `str(self)`, a string representation of `NearEarthObject`."""
...
class CloseApproach:
    ...
    Encapsulates information about the NEO's close approach to Earth,
    such as the date and time (in UTC) of closest approach, the nominal
    ...
    def __init__(self, designation, time, distance, velocity):
        ...
        :param velocity: (float) Velocity (km/s).
        """
        self.designation = designation
        assert \
            designation is not None and designation != '', \
            f"Attribute designation is required. Value:" \
            f"'{designation}' is not allowed"
